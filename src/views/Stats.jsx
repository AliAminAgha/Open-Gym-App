...
import { bwSheet, goalSheet, calendarSheet, workoutDetailSheet, WorkoutRow } from '../sheets.jsx'
...
import { prHistory } from '../lib/coach-tips.js'
import { calorieSuggestion, formatKcalRange } from '../lib/calories.js'
...
export default function Stats() {
  ...
  if (curEx) {
    S.workouts.forEach(w => {
      if (range > 0 && (w.start || new Date(w.d).getTime()) < now - range * 86400000) return
  ...
  const volPts = S.workouts
    .filter(w => range === 0 || (w.start || new Date(w.d).getTime()) > now - range * 86400000)
    .filter(w => (w.vol || 0) > 0)
    .map(w => ({ t: w.start || new Date(w.d).getTime(), y: w.vol, d: w.d }))
  ...
  const insight = streak >= 2
    ? <>{t('You\'re on a')} <b>{t('{0} week streak', streak)}</b> — {t('keep the rhythm going.')}</>
    : monthW > 0
      ? <>{t('You trained')} <b>{monthW}</b> {t('times this month')}.</>
      : t('Log a few workouts to unlock strength trends and personal records.')

  return <>
    <div className="hdr"><div><h1>{t('Progress')}</h1><div className="sub">{t('Strength, consistency & body')}</div></div>
    ...
    {hasProfile(S) && (() => {
      ...
      const rem = showGoal ? Math.round((goal - cur) * 10) / 10 : null
      const cals = calorieSuggestion(S.profile, currentWeightKg(S), S.unit)
      return (
        <div className="card">
          <h2 style={{ marginTop: 0 }}>{t('Your goal')} · {goalLabel(g)}</h2>
          {showGoal ? (
            <div className="goal-progress">
              <div>
                <div className="small muted">{t('Current')}</div>
                <div className="big" style={{ fontSize: 28 }}>{fmtNum(cur)} <span className="u">{S.unit}</span></div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div className="small muted">{t('Remaining')}</div>
                <div className="big" style={{ fontSize: 22, color: 'var(--acc)' }}>{fmtNum(Math.abs(rem))} {S.unit}</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div className="small muted">{t('Goal')}</div>
                <div className="big" style={{ fontSize: 28 }}>{fmtNum(goal)} <span className="u">{S.unit}</span></div>
              </div>
            </div>
          ) : (
            <div className="muted small">{t('Focus on strength trends and consistency below.')}</div>
          )}
          {cals && (
            <div className="small dim" style={{ marginTop: 12 }}>
              {t('Estimated intake')}: {formatKcalRange(cals.low, cals.high)}
              {cals.dailyDeficit != null && ` · ${t('Estimated deficit')}: ~${cals.dailyDeficit} kcal/day`}
              {cals.weeksToGoal ? ` · ${t('~{0} weeks to goal', cals.weeksToGoal)}` : ''}
              {' · '}{t('Estimates only')}
            </div>
          )}
        </div>
    ...
    <div className="cols">
      <div className="card">
        ...
        <Segmented className="seg-range" value={range} onChange={setRange}
          options={[{ value: 30, label: '1M' }, { value: 90, label: '3M' }, { value: 180, label: '6M' }, { value: 365, label: '1Y' }, { value: 0, label: t('All') }]} />
    ...
    {volPts.length > 1 && (
      <div className="card">
        <h2>{t('Volume progression')}</h2>
        <Segmented className="seg-range" value={range} onChange={setRange}
          options={[{ value: 30, label: '1M' }, { value: 90, label: '3M' }, { value: 180, label: '6M' }, { value: 365, label: '1Y' }, { value: 0, label: t('All') }]} />
        <div className="chart"><LineChart points={volPts} h={150} unit={S.unit} color="var(--acc)" /></div>
        <div className="small dim" style={{ marginTop: 8 }}>{t('Total load moved per workout')}</div>
      </div>
    )}

    {prHistory(S).length > 0 && (
      <div className="card">
        ...
        <div className="list" style={{ gap: 8 }}>
          {prHistory(S).map(p => (
            <div key={p.id} className="item" style={{ cursor: 'default' }}>
              ...
              <div className="grow">
                <div className="tt capitalize">{p.name}</div>
                <div className="ss">
                  {p.d ? fmtDate(p.d, true) + ' · ' : ''}{fmtNum(p.weight)} {S.unit}
                  {p.e1rm ? ` · e1RM ~${fmtNum(p.e1rm)}` : ''}
                </div>
              </div>
