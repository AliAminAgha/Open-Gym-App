...
export default function Login() {
  ...
  const head = <>
    <div style={{ fontSize: 56, display: 'flex', justifyContent: 'center', color: 'var(--acc)', filter: 'drop-shadow(0 0 24px color-mix(in srgb, var(--acc) 45%, transparent))' }}><Icon name="dumbbell" /></div>
    <h1 style={{ fontSize: 36, fontWeight: 700, letterSpacing: '-.04em', margin: '14px 0 6px' }}>Open Gym</h1>
  </>
  ...
  // Static PWA host (e.g. Vercel): no API — skip passkeys entirely.
  if (STATIC) return (
    <div className="narrow" style={wrap}>
      {head}
      <div className="muted" style={{ marginBottom: 30 }}>{t('Your workouts stay on this device. Install the app for one-tap access.')}</div>
      <Button variant="primary" icon="dumbbell" onClick={() => setGuest(true)}>{t('Get started')}</Button>
      <div className="card small muted" style={{ textAlign: 'left', marginTop: 16 }}>
        {t('No account needed. Export a backup from Settings anytime. Passkey sync needs a self-hosted Open Gym server.')}
      </div>
    </div>
  )
