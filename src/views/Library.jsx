  ...
  return (
    <>
      ...
      <div className="chips" style={{ marginBottom: eqOpts.length > 1 ? 8 : 14 }}>
        ...
        {BODYPARTS.map(b => (
          <button key={b} className={'chip' + (bp === b ? ' on' : '')} onClick={() => { setBp(b); setEq(''); setShown(40) }}>{t(b)}</button>
        ))}
      </div>
      ...
      {eqOpts.length > 1 && (
        <div className="chips" style={{ marginBottom: 14 }}>
          ...
          {eqOpts.map(x => (
            <button key={x} className={'chip' + (eqOn === x ? ' on' : '')} onClick={() => { setEq(x); setShown(40) }}>{t(x)}</button>
          ))}
        </div>
      )}
      ...
      <div className="lib-grid">
        {f.slice(0, shown).map(e => {
          ...
          return (
            <div key={e.id} className="lib-card" onClick={() => exerciseDetailSheet(e)}>
              <div className="media"><Thumb ex={e} /></div>
              <div className="body">
                <div className="tt">{e.n}</div>
                <div className="ss">{t(e.tg || e.bp)} · {t(e.eq)}</div>
                <div className="row between" style={{ marginTop: 10 }}>
                  {best > 0 ? <span className="tag acc">{fmtNum(best)} {S.unit}</span> : <span />}
                  <Button
                    size="sm"
                    variant="tinted"
                    icon="plus"
      ...
      {f.length > shown && <><div style={{ height: 14 }} /><Button onClick={() => setShown(s => s + 40)}>{t('Show more')}</Button></>}
    </>
  )
}
