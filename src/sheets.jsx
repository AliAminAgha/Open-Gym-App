...
function ExerciseDetail({ ex, close }) {
  ...
  const difficulty = /body weight|band/.test(eq) ? t('Beginner')
    : /barbell|olympic/.test(eq) ? t('Advanced')
      : t('Intermediate')
  return <>
    ...
    {(best > 0 || last) && (
      <div className="card" style={{ marginTop: 10, textAlign: 'left' }}>
        <div className="t-cap" style={{ marginBottom: 8 }}>{t('Your performance')}</div>
        {best > 0 && <div className="row between small" style={{ marginBottom: 6 }}>
          <span className="muted">{t('Personal best')}</span>
          <b className="accent">{fmtNum(best)} {st.unit}</b>
        </div>}
        {last && <div className="small muted">
          {t('Last time')} ({fmtDate(last.d)}): {last.sets.map(s => setLabel(ex.id, s, last.target)).join(' · ')}
        </div>}
      </div>
    )}
...
function FinishSummary({ w, prs, e1prs = [], close }) {
  ...
  const cmp = volumeCompare(w, st)
  const kcal = estimateWorkoutKcal(w, st)
  return <div className="finish-summary" style={{ textAlign: 'center', padding: '8px 0' }}>
    <div className={'finish-hero' + (prs.length ? ' has-pr' : '')}>
      <div style={{ fontSize: 48, display: 'flex', justifyContent: 'center', color: prs.length ? 'var(--yellow)' : 'var(--acc)' }}>
        <Icon name={prs.length ? 'trophy' : 'checkCircle'} />
      </div>
      <h3 style={{ margin: '10px 0 4px' }}>{prs.length ? t('New personal record!') : t('Workout complete!')}</h3>
      <div className="muted small">{w.name} · {fmtDate(w.d, true)}</div>
    </div>
    <div className="tiles" style={{ textAlign: 'left', marginTop: 14 }}>
      <div className="tile"><div className="l">{t('Duration')}</div><div className="v" style={{ fontSize: '1.15rem' }}>{fmtDur(w.end - w.start)}</div></div>
      <div className="tile"><div className="l">{t('Volume')}</div><div className="v" style={{ fontSize: '1.15rem' }}>{fmtVol(w.vol, st.unit)}</div></div>
      <div className="tile"><div className="l">{t('Sets')}</div><div className="v" style={{ fontSize: '1.15rem' }}>{setsDone(w)}</div></div>
      <div className="tile"><div className="l">{t('Est. kcal')}</div><div className="v" style={{ fontSize: '1.15rem' }}>{kcal != null ? `~${kcal}` : '—'}</div></div>
    </div>
    <div className="finish-highlights">
      <div className="t-cap" style={{ marginBottom: 8 }}>{t('Highlights')}</div>
      {prs.map(id => (
        <div key={id} className="hl"><Icon name="trophy" /><span>{t('New PR')}: {(EXIDX[id] || {}).n || id}</span></div>
      ))}
      {cmp && cmp.pct !== 0 && (
        <div className="hl">
          <Icon name={cmp.pct > 0 ? 'arrowUp' : 'arrowDown'} />
          <span>{cmp.pct > 0 ? '+' : ''}{cmp.pct}% {t('volume vs last session')}</span>
        </div>
      )}
      <div className="hl"><Icon name="check" /><span>{setsDone(w)} {t('sets completed')}</span></div>
      {kcal != null && (
        <div className="hl"><Icon name="flame" /><span>{t('Estimated calories burned')}: ~{kcal} kcal</span></div>
      )}
    </div>
    <div className="small dim" style={{ textAlign: 'left', marginBottom: 10 }}>{t('Calorie burn is an estimate, not a precise measurement.')}</div>
    {(prs.length > 0 || e1prs.length > 0) && <div className="pr-list" style={{ textAlign: 'left', marginBottom: 12 }}>
      {prs.map(id => <div key={id} className="pr-row capitalize"><Icon name="trophy" /><span>{t('New PR:')} {(EXIDX[id] || {}).n || id}</span></div>)}
      {e1prs.map(p => <div key={p.id} className="pr-row capitalize"><Icon name="chartLine" /><span>{t('Best estimated 1RM:')} {(EXIDX[p.id] || {}).n || p.id} · {fmtNum(p.est)} {st.unit}</span></div>)}
    </div>}
